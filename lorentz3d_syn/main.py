    ...
    for epoch in range(params.training.num_epochs):
        ...
        if params.compute_GV: 
            ...
            if (epoch >= params.training.generator_start_epoch and
                ...
                phi_upper_failing_mask_relax = phi_uppers > -1.0
        ...
        # Hybrid sampling: add direct sample-based losses
        sample_loss = torch.tensor(0.0, device=device)
        if n_samples_per_region > 0:
            x_init_s = sample_from_cells(region_cells['init'], n_samples_per_region)
            x_unsafe_s = sample_from_cells(region_cells['unsafe'], n_samples_per_region)
            x_outside_s = sample_from_cells(region_cells['outside'], n_samples_per_region)
            x_gen_s = sample_from_cells(region_cells['generator'], n_samples_per_region)

            if x_init_s.numel() > 0:
                sample_loss += F.relu(V_net(x_init_s).squeeze(-1) - 1.0).mean()
            if x_unsafe_s.numel() > 0:
                sample_loss += F.relu(params.constraints.beta_ra - V_net(x_unsafe_s).squeeze(-1)).mean()
            if x_outside_s.numel() > 0:
                sample_loss += F.relu(current_beta_s - V_net(x_outside_s).squeeze(-1)).mean()
            if x_gen_s.numel() > 0 and epoch >= params.training.generator_start_epoch:
                x_gen_s.requires_grad_(True)
        ...
        if params.compute_V:
            ...
            # Merge passing cells for all V regions periodically
            if ((epoch + 1) % 202 == 0):
                for region_name in ['outside', 'unsafe', 'init']:
                    if len(region_cells[region_name]) == 0:
                        continue

                    # Create relaxed failing mask for this region
                    if region_name == 'outside':
                        relaxed_failing_mask = bounds_updated['outside'][0] < (beta_s_check - 0.5)
                    elif region_name == 'unsafe':
                        relaxed_failing_mask = bounds_updated['unsafe'][0] < (params.constraints.beta_ra - 0.5)
                    elif region_name == 'init':
                        relaxed_failing_mask = bounds_updated['init'][1] > 1.5

                    merged_cells, num_merges = merge_passing_neighbor_cells(
                        region_cells[region_name],
                        relaxed_failing_mask,
                        max_passes=8,
                        max_merges=None,
                        seed=0,
                        eps=0.5,  # Relaxed tolerance for 3D merging
                    )

                    if num_merges > 0:
                        region_cells[region_name] = merged_cells
                        print(f"[Merge-{region_name.capitalize()}] Epoch {epoch+1}: merged {num_merges} pairs → {len(merged_cells)} total")
                        needs_cache_rebuild = True
        ...
        with torch.no_grad():
            ...
            if params.compute_V:
                show = (epoch % 10 == 0)
                # _, goal_satisfied = compute_loss_goal_bounds(V_net, regions.goal, bounds_updated["goal"][0], bounds_updated["goal"][1], 
                #                         beta_s_check, bounds_updated['outside'][0], device=device, show=show, check=True, n_samples=10000)
...
def main():
    ...
    params.network.input_scale = [60.0, 60.0, 60.0]
    ...
    # u_nn = LinearControlNN(prior_knowledge=True, input_dim=3)
    u_nn = NonlinearControlNN(hidden_dim=2) # 2500e with only 1 HL will get past loss 9.0
