"""
5D Point Mass with Orientation Control Synthesis
...
Goal: Navigate to origin with zero velocity and zero heading (stable equilibrium)
...
class PointMassControlNN(nn.Module):
    """
    Control network for 5D point mass with orientation.
    Maps state [x, y, vx, vy, θ] to control [u_x, u_y, u_θ].
    """
    def __init__(self, hidden_dim=32):
        super().__init__()
        self.fc1 = nn.Linear(5, hidden_dim, bias=True)
        self.fc2 = nn.Linear(hidden_dim, 16, bias=True)
        self.fc3 = nn.Linear(16, 3, bias=False)  # output: [u_x, u_y, u_θ]

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x: (N, 5) state [x, y, vx, vy, θ]
        returns: (N, 3) control [u_x, u_y, u_θ]
        """
...
def main():
    ...
    print("="*80)
    print("5D POINT MASS WITH ORIENTATION CONTROL SYNTHESIS")
    print("="*80)
    ...
    params.network.n_inputs = 5  # [x, y, vx, vy, θ]
    params.network.n_hidden_1 = 128
    params.network.n_hidden_2 = 16
    params.network.input_scale = [6.0, 6.0, 3.0, 3.0, 3.0]
    ...
    # Control network: 5 inputs → 3 outputs (accelerations + angular control)
    u_nn = InvertControlNN(input_dim=5, hidden_dim=8, output_dim=3)
    ...
    def f_ol(x: torch.Tensor) -> torch.Tensor:
        ...
        Equilibrium at origin: (0, 0, 0, 0, 0) with u = (0, 0, 0)
        """
        if x.dim() == 1:
            x = x.unsqueeze(0)
    ...
    g_coeffs = torch.tensor([0.1, 0.0, 0.1, 0.0, 0.0], dtype=torch.float32)

    def g(x: torch.Tensor) -> torch.Tensor:
        """
        Diffusion (diagonal, constant).
        Noise only on velocities and heading.
        """
    ...
    # Closed-loop drift: f_cl(x) = f_ol(x) + [0, 0, u_x, u_y, u_θ]
    class ClosedLoopDrift5D(nn.Module):
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            ...
            Returns: (N, 5) drift
            """
            f_open = self.f_ol_fn(x)  # (N, 5)
            u = self.controller(x)     # (N, 3)

            # CROWN-friendly: use torch.cat instead of indexing
            # f_open = [vx, vy, -0.3*vx, -0.3*vy, -0.5*θ]
            # control adds [0, 0, u_x, u_y, u_θ]
            # result = [vx, vy, -0.3*vx + u_x, -0.3*vy + u_y, -0.5*θ + u_θ]

            return torch.cat([
                f_open[:, 0:1],              # vx
                f_open[:, 1:2],              # vy
                f_open[:, 2:3] + u[:, 0:1],  # -0.3*vx + u_x
                f_open[:, 3:4] + u[:, 1:2],  # -0.3*vy + u_y
                f_open[:, 4:5] + u[:, 2:3],  # -0.5*θ + u_θ
            ], dim=1)
