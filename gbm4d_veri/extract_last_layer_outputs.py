...
def build_V(params: Hyperparameters, device: str = "cpu", pretrain=False, model_seed=None):
    ...
    if(pretrain):
        ckpt_path = OUTPUT_DIR / "4D_GBM_pretrained_V" / f"V_pretrained_seed_{model_seed}.pth"
        V_net.load_state_dict(torch.load(ckpt_path, map_location=device))
    else:
        sat_V_path = OUTPUT_DIR / "4D_GBM_sat_V" / f"V_seed_{model_seed}.pth"
        V_net.load_state_dict(torch.load(sat_V_path, map_location=device))
    V_net.eval()
    return V_net
...
def main():
    ...
    ## Pre-processing (saved time and memory, I run it once, don't need to run anymore)
    # for i in range(5):
    #     model_seed = i
    #     bundle_path = OUTPUT_DIR / "4D_GBM_sat_V" / f"eval_bundle_seed_{model_seed}.pth"
    #     bundle = load_eval_bundle(bundle_path, map_location="cpu")
    #     V_state = bundle["V_state_dict"]
    #     torch.save(V_state, OUTPUT_DIR / "4D_GBM_sat_V" / f"V_seed_{model_seed}.pth")
    # return
