"""
4D Double Pendulum Control Synthesis
Double pendulum without cart (pure acrobot)
...
Unsafe: Horizontal angles or high angular velocities
"""
import torch
...
class AcrobotControlNN(nn.Module):
    """
    Control network for 4D double pendulum (pure acrobot).
    Maps state [θ1, θ2, ω1, ω2] to control τ (torque at elbow).
    ...
    def __init__(self, hidden_dim=32):
        ...
        self.fc3 = nn.Linear(16, 1, bias=False)  # output: τ

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x: (N, 4) state [θ1, θ2, ω1, ω2]
        returns: (N, 1) control τ
        """
        h1 = F.tanh(self.fc1(x))
        h2 = F.tanh(self.fc2(h1))
        u = 1.0 * F.tanh(self.fc3(h2))  # bound control
        return u
...
def main():
    ...
    print("="*80)
    print("4D DOUBLE PENDULUM CONTROL SYNTHESIS")
    print("="*80)
    ...
    params.network.n_inputs = 4  # [θ1, θ2, ω1, ω2]
    params.network.n_hidden_1 = 128
    params.network.n_hidden_2 = 16
    params.network.input_scale = [3.0, 3.0, 3.0, 3.0]  # scale angles/velocities
    ...
    # Control network: 4 inputs → 1 output (torque only)
    # u_nn = InvertControlNN(input_dim=4, hidden_dim=1024, output_dim=1)
    u_nn = AcrobotControlNN(hidden_dim=16)
    ...
    def f_ol(x: torch.Tensor) -> torch.Tensor:
        """
        Open-loop drift for 4D double pendulum (pure acrobot).
        x: (N, 4) = [θ1, θ2, ω1, ω2]
        returns: (N, 4) drift
        ...
        Stable equilibrium: [0, 0, 0, 0] (hanging down)
        Unstable equilibrium: [π, 0, 0, 0] (upright) - GOAL!
        """
        if x.dim() == 1:
            x = x.unsqueeze(0)
    ...
    g_coeffs = torch.tensor([0.0, 0.0, 0.05, 0.05], dtype=torch.float32)

    def g_fn(x: torch.Tensor) -> torch.Tensor:
        """
        Diffusion (diagonal, constant).
        Noise on angular velocities only.
        """
    ...
    # Closed-loop drift: adds control torque to elbow
    class ClosedLoopDrift4D(nn.Module):
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            ...
            u = self.controller(x)  # (N, 1) = τ_elbow
            tau_elbow = u[:, 0]
    ...
    f_cl_module = ClosedLoopDrift4D(f_ol, u_nn).to(params.training.device)
   ...
   # Init: start near hanging down (stable equilibrium)
    ...
    goal_range = np.array([
        [ 2.9,  3.14159],   # θ1 near π (upright, unstable!)
        [-0.3,  0.3],       # θ2 near 0 (aligned)
        [-0.3,  0.3],       # ω1 near 0 (balanced)
        [-0.3,  0.3],       # ω2 near 0 (balanced)
    ], dtype=np.float32)

    # Unsafe: extreme backward swing with high negative velocities (out of control)
    # This doesn't block the path from init (θ1≈0) to goal (θ1≈π)
    unsafe_range = np.array([
        [-3.2, -2.5],   # θ1 far negative (backward)
        [-2.5, -1.0],   # θ2 also negative (folded back)
        [-2.0, -1.5],   # ω1 large negative velocity
        [-2.0, -1.5],   # ω2 large negative velocity
    ], dtype=np.float32)

    # Full range
    full_range = np.array([
        [-3.2,  3.2],   # θ1 (±π approximately)
        [-3.2,  3.2],   # θ2
        [-2.0,  2.0],   # ω1
        [-2.0,  2.0],   # ω2
    ], dtype=np.float32)
    ...
    if args.train == 1:
        ...
        u_nn = InvertControlNN(input_dim=4, hidden_dim=512, output_dim=1)
