"""
Test and visualize 4D double pendulum control synthesis
...
# ========================================================================
NOISE_DIAG = np.array([0.0, 0.0, 0.1, 0.1], dtype=float)
...
def f_np(x: np.ndarray, u: np.ndarray) -> np.ndarray:
    """
    Closed-loop drift for pure acrobot dynamics.
    x: (4,) = [θ1, θ2, ω1, ω2]
    u: (1,) = τ_elbow
    returns: (4,) drift
    """
...
def load_control_net(bundle_path, device="cpu"):
    ...
    control_net = InvertControlNN(input_dim=4, hidden_dim=512, output_dim=1)
...
def get_u(x_vec: np.ndarray, controller) -> np.ndarray:
    ...
    if u.size != 1:
        raise ValueError(f"controller must return shape (1,), got {u.shape}")
    return u
...
# PENDULUM KINEMATICS
# ========================================================================
def compute_pendulum_positions(theta1, theta2):
    """
    ...
    - (x0, y0): base position (fixed at origin)
    - (x1, y1): end of link 1
    - (x2, y2): end of link 2
    """
    x0, y0 = 0.0, 0.0
...
def test_single_traj_run(controller=None, T=20.0, seed=None):
    ...
    u_hist = np.zeros((N, 1), dtype=float)
    ...
    print(f"\nFinal state: θ1={x[-1,0]:.3f}, θ2={x[-1,1]:.3f}, ω1={x[-1,2]:.3f}, ω2={x[-1,3]:.3f}")
    print("="*80 + "\n")
    ...
    ax_control = fig.add_subplot(gs[:, 2]) # control inputs vs time (right, spans both rows)

    title = "4D Double Pendulum (u=0)" if controller is None else "4D Double Pendulum (controlled)"
    ...
    # Unsafe regions
    ax_angles.axhspan(unsafe_range[0,0], unsafe_range[0,1], color='red', alpha=0.2, label='θ1 unsafe')
    ax_angles.axhspan(unsafe_range[1,0], unsafe_range[1,1], color='salmon', alpha=0.2, label='θ2 unsafe')
    ...
    # Unsafe regions
    ax_vels.axhspan(unsafe_range[2,0], unsafe_range[2,1], color='red', alpha=0.2, label='ω1 unsafe')
    ax_vels.axhspan(unsafe_range[3,0], unsafe_range[3,1], color='salmon', alpha=0.2, label='ω2 unsafe')
    # Goal regions
    ax_vels.axhspan(goal_range[2,0], goal_range[2,1], color='green', alpha=0.15, label='ω1 goal')
    ax_vels.axhspan(goal_range[3,0], goal_range[3,1], color='lightgreen', alpha=0.15, label='ω2 goal')
    ...
    if controller is not None:
        ...
        ax_control.set_ylabel("Control torque τ")
        ...
        traj_u_tau, = ax_control.plot([], [], 'b-', lw=1.5, label='τ (torque)')
        ax_control.legend(loc='upper right', fontsize=8)
    else:
        ax_control.set_xlim(0, T)
        ax_control.set_xlabel("Time (s)")
        ax_control.set_ylabel("Control torque τ")
        ax_control.set_title("Control Torque (u=0)")
        ax_control.grid(True, alpha=0.3)
        ax_control.axhline(0, color='black', linestyle='-', alpha=0.3, linewidth=0.5)
        traj_u_tau, = ax_control.plot([], [], 'b-', lw=1.5, label='τ (torque)')
    ...
    def init_anim():
        ...
        artists = [pendulum_line, base_pt, trail, traj_theta1, traj_theta2,
                   traj_omega1, traj_omega2, traj_u_tau, time_text_pend, status_text]
        return artists

    def update(frame):
        theta1, theta2, omega1, omega2 = x[frame]

        # Compute pendulum positions
        (x0, y0), (x1, y1), (x2, y2) = compute_pendulum_positions(theta1, theta2)
        ...
        # Check status
        x_vec = np.array([theta1, theta2, omega1, omega2], dtype=float)
        ...
        artists = [pendulum_line, base_pt, trail, traj_theta1, traj_theta2,
                   traj_omega1, traj_omega2, time_text_pend, status_text]

        # Update control plot
        traj_u_tau.set_data(t_grid[:frame+1], u_hist[:frame+1, 0])
        # Auto-scale control axes
        if frame > 0 and controller is not None:
            u_max = np.abs(u_hist[:frame+1, 0]).max()
            ax_control.set_ylim(-u_max*1.1, u_max*1.1)
        artists.append(traj_u_tau)
...
def test_mc(controller=None):
    ...
    print("Reach-avoid MC estimate (4D Double Pendulum):")
