"""
5D Acrobot-Inspired Control Synthesis
Double pendulum with movable base (inspired by Gymnasium Acrobot)

State: x = [θ1, θ2, ω1, ω2, x_base]
- θ1: angle of first link (from downward vertical, rad)
- θ2: angle of second link (relative to first link, rad)
- ω1: angular velocity of first link (rad/s)
- ω2: angular velocity of second link (rad/s)
- x_base: horizontal position of base anchor point (m)

Goal: Balance at upright position (θ1≈π, θ2≈0) - UNSTABLE equilibrium!
Init: Start near hanging down (θ1≈0, θ2≈0) - stable equilibrium
Unsafe: Horizontal angles or base far from origin
"""
import torch
...
class AcrobotControlNN(nn.Module):
    """
    Control network for 5D acrobot with movable base.
    Maps state [θ1, θ2, ω1, ω2, x_base] to control [τ, F].
    τ: torque at joint between links
    F: horizontal force on base
    """
    def __init__(self, hidden_dim=32):
        super().__init__()
        self.fc1 = nn.Linear(5, hidden_dim, bias=True)
        self.fc2 = nn.Linear(hidden_dim, 16, bias=True)
        self.fc3 = nn.Linear(16, 2, bias=False)  # output: [τ, F]

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        x: (N, 5) state [θ1, θ2, ω1, ω2, x_base]
        returns: (N, 2) control [τ, F]
        """
        h1 = F.tanh(self.fc1(x))
        h2 = F.tanh(self.fc2(h1))
        u = 100.0 * F.tanh(self.fc3(h2))  # bound control
        return u
...
def main():
    ...
    print("="*80)
    print("5D ACROBOT-INSPIRED CONTROL SYNTHESIS")
    print("="*80)
    ...
    params.network.n_inputs = 5  # [θ1, θ2, ω1, ω2, x_base]
    params.network.n_hidden_1 = 128
    params.network.n_hidden_2 = 16
    params.network.input_scale = [3.0, 3.0, 3.0, 3.0, 6.0]  # scale angles/velocities, position more
    ...
    # Control network: 5 inputs → 2 outputs (torque + force)
    u_nn = InvertControlNN(input_dim=5, hidden_dim=512, output_dim=2)

    # Physical parameters (following MIT Underactuated Robotics notation)
    # Simplified: assume uniform rods
    g = 9.8      # gravity
    m1 = 1.0     # mass of link 1
    m2 = 1.0     # mass of link 2
    l1 = 1.0     # length of link 1
    l2 = 1.0     # length of link 2
    lc1 = 0.5    # distance to center of mass of link 1 (l1/2)
    lc2 = 0.5    # distance to center of mass of link 2 (l2/2)
    # Moments of inertia: I = m*l^2/3 for rod rotating about end
    I1 = m1 * l1**2 / 3.0
    I2 = m2 * l2**2 / 3.0

    # Damping coefficients (moderate to prevent instability)
    d1 = 0.5     # damping on shoulder joint
    d2 = 0.5     # damping on elbow joint
    d_base = 0.3 # damping on base

    def f_ol(x: torch.Tensor) -> torch.Tensor:
        """
        Open-loop drift for 5D acrobot with movable base.
        x: (N, 5) = [θ1, θ2, ω1, ω2, x_base]
        returns: (N, 5) drift

        Acrobot dynamics from MIT Underactuated Robotics:
        M(q)q̈ + C(q,q̇)q̇ = τ_g(q) + B*u

        State: q = [θ1, θ2], q̇ = [ω1, ω2]
        Control on elbow joint only (underactuated)

        Stable equilibrium: [0, 0, 0, 0, 0] (hanging down)
        Unstable equilibrium: [π, 0, 0, 0, 0] (upright) - GOAL!
        """
        if x.dim() == 1:
            x = x.unsqueeze(0)
        ...
        # Determinant for inversion (add small epsilon for numerical stability)
        det_M = M11 * M22 - M12 * M21
        det_M = torch.clamp(det_M, min=1e-6)  # Prevent division by zero/negative
        ...
        C12 = -h * omega2
        C21 = h * omega1
        C22 = torch.zeros_like(omega1)

        # Gravity torques τ_g(q)
        # Positive torque causes positive angular acceleration (counterclockwise)
        # When θ=0 (hanging), gravity should not create torque
        # When θ>0, gravity should pull back toward hanging (negative torque)
        ...
        # Solve for accelerations: q̈ = M^{-1}(τ_g - C*q̇ - damping)
        # Add damping: -d1*ω1 on shoulder, -d2*ω2 on elbow
        rhs1 = tau_g1 - C_term1 - d1 * omega1
        rhs2 = tau_g2 - C_term2 - d2 * omega2

        # Invert M and compute accelerations
        alpha1 = (M22 * rhs1 - M12 * rhs2) / det_M
        alpha2 = (-M21 * rhs1 + M11 * rhs2) / det_M

        # Base dynamics: simple damped system
        # The base is weakly coupled through center of mass shifts (simplified)
        dx_base = -d_base * x_base

        return torch.stack([
            omega1,    # dθ1/dt = ω1
            omega2,    # dθ2/dt = ω2
            alpha1,    # dω1/dt from acrobot dynamics
            alpha2,    # dω2/dt from acrobot dynamics
            dx_base    # dx_base/dt damped to origin
        ], dim=1)

    # Constant diagonal diffusion
    g_coeffs = torch.tensor([0.0, 0.0, 0.1, 0.1, 0.05], dtype=torch.float32)

    def g_fn(x: torch.Tensor) -> torch.Tensor:
        """
        Diffusion (diagonal, constant).
        Noise on angular velocities and base position.
        """
    ...
    # Closed-loop drift: adds control torque to elbow and force to base
    class ClosedLoopDrift5D(nn.Module):
        ...
        def forward(self, x: torch.Tensor) -> torch.Tensor:
            """
            x: (N, 5) = [θ1, θ2, ω1, ω2, x_base]
            u: (N, 2) = [τ_elbow, F_base]
            Returns: (N, 5) drift

            Standard acrobot: control torque only at elbow (underactuated)
            Plus: horizontal force on base
            """
            if x.dim() == 1:
                x = x.unsqueeze(0)
            ...
            u = self.controller(x)  # (N, 2) = [τ_elbow, F_base]
            tau_elbow = u[:, 0:1]
            F_base = u[:, 1:2]

            # Compute open-loop accelerations (without control)
            # Trigonometric shortcuts
            s1 = torch.sin(theta1)
            s2 = torch.sin(theta2)
            s12 = torch.sin(theta1 + theta2)
            ...
            M22 = I2
            det_M = M11 * M22 - M12 * M21
            # Use maximum instead of clamp for auto_LiRPA compatibility
            det_M = torch.maximum(det_M, torch.tensor(1e-6, dtype=det_M.dtype, device=det_M.device))
            ...
            # Shoulder: no direct control (underactuated), only damping
            # Elbow: control torque τ_elbow applied here
            rhs1 = tau_g1 - C_term1 - d1 * omega1  # no control on shoulder
            rhs2 = tau_g2 - C_term2 - d2 * omega2 + tau_elbow.squeeze(-1)  # control on elbow

            # Solve M*α = rhs for accelerations
            alpha1 = (M22 * rhs1 - M12 * rhs2) / det_M
            alpha2 = (-M21 * rhs1 + M11 * rhs2) / det_M

            # Base dynamics with control force
            dx_base = -d_base * x_base + F_base.squeeze(-1)

            return torch.stack([
                omega1,     # dθ1/dt = ω1
                omega2,     # dθ2/dt = ω2
                alpha1,     # dω1/dt from acrobot dynamics
                alpha2,     # dω2/dt from acrobot dynamics (with control)
                dx_base     # dx_base/dt with force control
            ], dim=1)

    f_cl_module = ClosedLoopDrift5D(f_ol, u_nn).to(params.training.device)
    dynamics = Dynamics.dynamics(f=f_cl_module, g=g_fn)
   ...
   # Init: start near hanging down (stable equilibrium)
    ...
    if args.train == 1:
        ...
        u_nn = InvertControlNN(input_dim=5, hidden_dim=8, output_dim=2)
