...
# ----------------------------
# Region specs from main code (3D)
# ----------------------------
init_range = np.array([
    [45.0, 55.0],
    [-55.0, -45.0],
...
def _draw_proj_rect(
    ax, x_bounds, y_bounds, *, label=None, alpha=0.18, lw=1.5, fill=True, linestyle="--",
...
# ----------------------------
# Dynamics (numpy) - 3D chain drift + diagonal diffusion
# ----------------------------
...
def test_single_traj_run(controller=None, T=10.0, seed=None, n_traj: int = 10):
    """
    Multi-traj SDE rollout + animation (3D).
    controller:
      - None -> u(x)=0 in R^3
    ...
    # State history: (N, n_traj, 3)
    x = np.zeros((N, n_traj, X_DIM), dtype=float)
    x[0] = x0
    print("x0[0] =", x0[0])

    # Control history: (N, n_traj, 3)
    ...
    for k in range(N - 1):
        ...
        for i in range(n_traj):
            u = get_u(x_curr[i], controller=controller)  # (3,)
            u_hist[k, i] = u

            drift = f_np(x_curr[i], u)     # (3,)
            g_vec = g_diag_np(x_curr[i])   # (3,)

            dW = np.sqrt(dt) * rng.standard_normal(size=X_DIM)  # (3,)
    ...
    title = "3D GBM (closed loop with u_i = -x_i)" if controller is None else "3D GBM (controlled)"
    ...
    _draw_proj_rect(
        ax32, init_range[2], init_range[1],
        label=r"$X_{\mathrm{init}}$", fill=True, alpha=0.18, linestyle="--", lw=1.5,
        facecolor="green", edgecolor="green"
    )
    ...
    _draw_proj_rect(
        ax32, goal_range[2], goal_range[1],
        label=r"$X_{\mathrm{goal}}$", fill=True, alpha=0.20, linestyle="-", lw=1.5,
        facecolor="blue", edgecolor="blue"
    )
    ...
    _draw_proj_rect(
        ax32, unsafe_range[2], unsafe_range[1],
        label=r"$X_{\mathrm{unsafe}}$", fill=True, alpha=0.25, linestyle="-", lw=1.5,
        facecolor="red", edgecolor="red"
    )
    ...
    def init_anim():
        ...
        artists = []
        for i in range(n_traj):
            artists += [traj12_list[i], pt12_list[i], traj32_list[i], pt32_list[i]]
    ...
    def update(frame):
        ...
        for i in range(n_traj):
            xi = x[:frame+1, i, :]  # (frame+1, 3)
            x1, x2, x3 = xi[-1]

            traj12_list[i].set_data(xi[:, 0], xi[:, 1])
            pt12_list[i].set_data([x1], [x2])

            traj32_list[i].set_data(xi[:, 2], xi[:, 1])
            pt32_list[i].set_data([x3], [x2])

        # Status text based on traj 0 (keeps UI simple)
        x_vec0 = x[frame, 0, :].astype(float)
        in_goal0 = in_box_3d(x_vec0, goal_range)
        in_unsafe0 = in_box_3d(x_vec0, unsafe_range)
...
def estimate_reach_avoid_mc(
    ...
    for r in range(n_mc):
        ...
        for k in range(N_mc - 1):
            ...
            dW = np.sqrt(dt_mc) * rng_mc.standard_normal(size=X_DIM)  # (3,)
