...
def run_rollout_case(title, controller, d, mode="uniform", T=30.0, seed=1, plot_uncertainty=True):
    print(f"\n=== {title} ===")
...
def run_mc_case(title, controller, d, mode, n_mc=1000, T_mc=8.0, dt_mc=0.005, seed_mc=1):
    print(f"\n=== {title} ===")
...
def main():
    ...
    run_rollout_case("1) Uncontrolled rollout (additive-uniform)", None, d, mode="uniform")
    run_rollout_case("2) Controlled rollout (additive-uniform)", control_net, d, mode="uniform")
    run_mc_case("3) MC (additive-uniform, controlled)", control_net, d, mode="uniform")
    run_mc_case("4) MC (additive-corners, controlled)", control_net, d, mode="corners")
