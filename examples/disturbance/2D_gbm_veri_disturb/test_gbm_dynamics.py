...
def main():
    ...
    # -------------------------------------------------------------------------
    # CASE 1) additive bounded disturbance
    # -------------------------------------------------------------------------
    test_single_traj_run(
        controller=controller,
        V_net=V_net,
        unc_type="additive",
        mode="uniform",          # "uniform" | "corners" | "worstcase_V" | "none"
        d=np.array([10.0, 10.0], dtype=float),
        T=10.0,
        seed=0,
        device="cpu",
        plot_uncertainty=True,
    ...
    for mode in ["none", "uniform", "corners"]:
        test_mc(
            controller=controller,
            V_net=V_net,
            unc_type="additive",
            mode=mode,
            d=np.array([10.0, 1.0], dtype=float),
            n_mc=1000,
            T_mc=8.0,
            dt_mc=0.005,
            seed_mc=0,
