...
from src.set_values import LinearIntervalSetDrift
...
def main(benchmark_mode=False):
    ...
    # Merging
    params.refinement.v_outside.enable_merging = False
    ...
    # Merging
    params.refinement.gv_generator.enable_merging = False
    ...
    # Set-valued open-loop drift: f_nom(x) + w, w in [-d,d]
    # drift_unc = torch.tensor([10.0, 10.0], dtype=torch.float32)  # per-dimension
    # f_ol_set = AdditiveBoxSetDrift(f_ol, drift_unc).to(params.training.device)

    # Set-valued open-loop drift
    ...
    print(A_L)
    print(A_U)
    f_ol_set = LinearIntervalSetDrift(A_L, A_U).to(params.training.device)
    dynamics = Dynamics.dynamics(f=f_ol_set, g=g, state_dim=params.network.n_inputs)
