"""Curriculum-based 2D inverted-pendulum synthesis (non-temporal)."""
...
def main(benchmark_mode: bool = False):
    ...
    parser.add_argument("--deterministic", type=int, default=1, choices=[0, 1],
                        help="1: deterministic training (slower), 0: faster non-deterministic")
    args = parser.parse_args()
    if benchmark_mode:
        args.benchmark = 1

    print("=" * 20)
    print("Inverted Pendulum Synthesis (Curriculum + Coarse-to-Fine)")
    print("=" * 20)

    params = configure_default_params()
    set_global_reproducibility(int(params.training.random_seed), deterministic=bool(args.deterministic))
    print(
        f"[reproducibility] seed={params.training.random_seed}, "
        f"deterministic_algorithms={bool(args.deterministic)}"
    )
    ...
    run_load_path(bundle_path=bundle_path)
    return None
