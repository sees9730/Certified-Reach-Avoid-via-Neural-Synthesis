"""Workflow helpers for curriculum-based inverted-pendulum synthesis."""
...
def set_global_reproducibility(seed: int, deterministic: bool = True) -> None:
    """Configure randomness; deterministic mode is optional because it is slower."""
...
def make_controller(device: str) -> torch.nn.Module:
    """Create state-feedback controller for synthesis."""
    return WrapperConterlNN(InvertControlNN(hidden_dim=8)).to(device)


def configure_default_params() -> Hyperparameters:
    """Build a curriculum-focused, coarse-to-fine synthesis configuration."""
    ...
    params.training.device = _auto_device()
    params.training.learning_rate = 0.005
    params.training.num_epochs = 120000
    params.training.generator_weight = 1.0
    params.training.generator_start_epoch = 0
    params.training.generator_after_v_sat = False
    params.training.generator_ramp_epochs = 0 # makes generator_weight_k ramp from 0 to params.training.generator_weight over 200 epochs.
    params.training.use_generator_threshold_curriculum = True
    params.training.generator_threshold_init = 4.0
    params.training.generator_threshold_final = 0.0
    params.training.generator_threshold_step = 0.5
    params.training.generator_threshold_step_fine = 0.1
    params.training.generator_threshold_switch = 0.8

    params.logging.visualize_interval = 0
    params.logging.detailed_eval_interval = 500

    # Coarse initial cells; refinement expands only where constraints fail.
    params.discretization.n_goal = [8, 8]
    params.discretization.n_outside_goal = [2, 2]
    params.discretization.n_generator = [2, 2]
    params.discretization.n_unsafe = [8, 8]
    params.discretization.n_init = [8, 8]

    params.constraints.beta_ra = 20.0
    params.constraints.use_beta_ra_curriculum = True
    params.constraints.beta_ra_init = 3.0
    ...
    params.training.enable_smt_cell_filter = True
    params.training.smt_cell_filter_start_epoch = 0
    params.training.smt_cell_filter_interval = 100
    params.training.smt_cell_filter_max_cells = 16
    params.training.smt_cell_filter_timeout_ms = 100
    params.training.smt_cell_filter_regions = ["generator"]
    params.training.smt_check_all_regions_in_stage3 = True
    # Stage-3 full SMT (all constraints) cadence: keep bound checks every epoch,
    # but run expensive full SMT checks periodically.
    params.training.stage3_smt_full_check_interval = 100
    params.training.stage3_smt_full_check_start_epoch = 0
    params.training.smt_parallel_workers = 4
    params.training.enable_smt_counterexample_replay = True
    params.training.smt_ce_replay_max_points_per_region = 256
    params.training.smt_ce_replay_batch_size = 32
    params.training.smt_ce_replay_noise_radius = 0.02
    params.training.smt_ce_replay_weight_v = 0.2
    params.training.smt_ce_replay_weight_gv = 0.2

    params.refinement.v_goal.enable_refinement = True
    params.refinement.v_goal.refine_factor = [2, 2]
    ...
    params.refinement.v_unsafe.refine_factor = [2, 2]
    ...
    params.refinement.v_init.refine_factor = [2, 2]
    ...
    params.refinement.v_outside.refine_factor = [2, 2]
    ...
    params.refinement.gv_generator.refine_factor = [2, 2]
    ...
    params.refinement.simple_regions_per_trigger = 4
...
def build_regions() -> Tuple[Regions, np.ndarray, np.ndarray, np.ndarray]:
    """Create init/goal/unsafe/full regions for 2D synthesis."""
    pi = np.pi
    ...
    unsafe_down1 = np.array([[-2 * pi, -1.5 * pi], [-20.0, -10.0]], dtype=np.float32)
    unsafe_down2 = np.array([[1.5 * pi, 2 * pi], [10.0, 20.0]], dtype=np.float32)
...
def run_load_path(bundle_path: Path) -> None:
