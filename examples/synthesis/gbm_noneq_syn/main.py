...
from test_gbm_noneq_dynamics import main as test_dyn_main
...
def main(benchmark_mode=False):
    ...
    params.refinement.v_outside.N_to_refine = 200
    ...
    params.refinement.v_outside.merge_relax_margin = 1.5
    ...
    params.refinement.gv_generator.N_to_refine = 200
    ...
    goal_range = np.array([[20.0, 40.0], [-25.0, 25.0]], dtype=np.float32)
    ...
    unsafe = Region.union(unsafe_obs, unsafe_tb, unsafe_bb, unsafe_lb, unsafe_rb)
    full = Region(full_range)

    regions = Regions(init=init, goal=goal, unsafe=unsafe, full=full)

    # === Networks ===
    input_offset = [30.0, 0.0]
    ...
    if args.train == 1:
        ...
        print("\n" + "="*20)
        print("Calling test_gbm_noneq_dynamics.main() ...")
        print("="*20)
        test_dyn_main()  # run the other file's main()
