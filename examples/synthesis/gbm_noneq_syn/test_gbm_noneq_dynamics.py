...
from animate_minimal import animate_gbm_minimal
...
class ClosedLoopDriftModule(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        f = torch.stack([dx1_dt, dx2_dt], dim=1)
        return f + u
...
def g_diffusion(x: torch.Tensor) -> torch.Tensor:
    """Diffusion matrix g(x) - returns 2x2 matrix."""
    x1 = x[:, 0:1]
    x2 = x[:, 1:2]

    # Create 2x2 diagonal matrix for each sample
    batch_size = x.shape[0]
    g_mat = torch.zeros(batch_size, 2, 2, dtype=x.dtype, device=x.device)
    g_mat[:, 0, 0] = (0.2 * x1).squeeze()
    g_mat[:, 1, 1] = (0.2 * x2).squeeze()

    # For single sample, squeeze batch dimension
    if batch_size == 1:
        return g_mat.squeeze(0)
    return g_mat
...
def main():
    ...
    # Load bundle to get V_net
    bundle = load_eval_bundle(OUTPUT_DIR / "eval_bundle.pth", map_location="cpu")
    ...
    # Get input_offset and output_offset from bundle if available
    input_offset = None
    output_offset = None
    if "V_state_dict" in bundle:
        state_dict = bundle["V_state_dict"]
        if "input_offset" in state_dict:
            input_offset = state_dict["input_offset"].cpu().numpy().tolist()
        if "output_offset" in state_dict:
            output_offset = float(state_dict["output_offset"].cpu().numpy())
    ...
    animate_gbm_minimal(
        ...
        g_fn=g_diffusion,
        V_net=V_net,
        init_range=X_init_bounds,
        goal_range=X_goal_bounds,
        full_range=X_bounds,
        unsafe_range=X_unsafe_1,
        device="cpu",
        dt=0.01,
        T=10.0,
        seed=0,
        ...
        show=True,
        controller_label="2D GBM Non Equilibrium Goal",
        n_trajectories=5
    )
