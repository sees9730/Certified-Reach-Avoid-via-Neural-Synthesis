...
from src.control_network import Veh4DLinearControlNN, Veh4DControlNN, Wrapper4DConterlNN
...
from src.validate_dynamics import estimate_reach_avoid_mc
        ...
        if params.compute_V and len(bounds_updated['unsafe'][0]) > 0:
            ...
            if num_unsafe_failing > 0:
                ...
                if (epoch == 1 or (epoch + 1) % REFINE_INTERVAL == 0) and \
                    ...
                    needs_cache_rebuild = True
            if((epoch + 1) % 501 == 0):
                unsafe_failing_mask_relax = bounds_updated['unsafe'][0] < params.constraints.beta_ra*20.0
                merged_cells, num_merges = merge_passing_neighbor_cells(
                    region_cells['unsafe'],
...
def main():
    ...
    params.network.scale_factor = 2.0
    ...
    control_policy = Veh4DControlNN()
    u_nn = Wrapper4DConterlNN(control_policy, U_max=10)
    ...
    g_coeffs = torch.tensor([0.0, 0.0, 0.02, 0.02], dtype=torch.float32)
    ...
    unsafe_lastb = np.array([
        [-10.0, 10.0],
        [-10.0, 10.0],
        [-10.0, 10.0], 
        [-10.0, -10.0+0.5], 
    ], dtype=np.float32)
    unsafe_lastbb = np.array([
        [-10.0, 10.0],
        [-10.0, 10.0],
        [-10.0, 10.0], 
        [10.0-0.5, 10.0], 
    ...
    unsafe_lastb = Region(unsafe_lastb)
    unsafe_lastbb = Region(unsafe_lastbb)
    ...
    V_net = create_V(params.network, input_offset=input_offset, output_offset=0.1)
    GV_net = create_GV(
        V_net=V_net,
        dynamics=dynamics,
    ...
    if args.train == 1:
        ...
        ENABLE_PRETRAINING = False  # Set to True to enable
        ...
        control_policy = Veh4DLinearControlNN()
        u_ol = Wrapper4DConterlNN(control_policy, U_max=1)
        u_ol.load_state_dict(torch.load(OUTPUT_DIR / "controller_pretrained.pth", map_location=device))
        f_ol_module = ClosedLoopDrift(f_ol, u_ol).to(params.training.device)
        ...
        print("SAT-controlled reach-avoid prob (MC): ", p_ra_mc)
